...
class ProteinFeatures(nn.Module):
    ...
    def forward(self, X, mask, residue_idx, chain_labels):
        ...
        RBF_all.append(self._get_rbf(Ox, Ox, E_idx))  # O-O
        RBF_all.append(self._get_rbf(Cb, Cb, E_idx))  # Cb-Cb
        RBF_all.append(self._get_rbf(Ca, N, E_idx))  # Ca-N
        RBF_all.append(self._get_rbf(Ca, C, E_idx))  # Ca-C
        RBF_all.append(self._get_rbf(Ca, Ox, E_idx))  # Ca-O
        RBF_all.append(self._get_rbf(Ca, Cb, E_idx))  # Ca-Cb
        RBF_all.append(self._get_rbf(N, C, E_idx))  # N-C
        RBF_all.append(self._get_rbf(N, Ox, E_idx))  # N-O
        RBF_all.append(self._get_rbf(N, Cb, E_idx))  # N-Cb
        RBF_all.append(self._get_rbf(Cb, C, E_idx))  # Cb-C
        RBF_all.append(self._get_rbf(Cb, Ox, E_idx))  # Cb-O
        RBF_all.append(self._get_rbf(Ox, C, E_idx))  # O-C
        RBF_all.append(self._get_rbf(N, Ca, E_idx))  # N-Ca
        RBF_all.append(self._get_rbf(C, Ca, E_idx))  # C-Ca
        RBF_all.append(self._get_rbf(Ox, Ca, E_idx))  # O-Ca
        RBF_all.append(self._get_rbf(Cb, Ca, E_idx))  # Cb-Ca
        RBF_all.append(self._get_rbf(C, N, E_idx))  # C-N
        RBF_all.append(self._get_rbf(Ox, N, E_idx))  # O-N
        RBF_all.append(self._get_rbf(Cb, N, E_idx))  # Cb-N
        RBF_all.append(self._get_rbf(C, Cb, E_idx))  # C-Cb
        RBF_all.append(self._get_rbf(Ox, Cb, E_idx))  # O-Cb
        RBF_all.append(self._get_rbf(C, Ox, E_idx))  # C-O
...
class CA_ProteinFeatures(nn.Module):
    ...
    def _orientations_coarse(self, X, E_idx, eps=1e-6):
        ...
        E = torch.stack((o_1, n_2, torch.cross(o_1, n_2)), 2)
        E = E.view(list(E.shape[:2]) + [9])
        E = F.pad(E, (0, 0, 1, 2), "constant", 0)
        O_neighbors = gather_nodes(E, E_idx)
        X_neighbors = gather_nodes(X, E_idx)

        # Re-view as rotation matrices
        E = E.view(list(E.shape[:2]) + [3, 3])
        O_neighbors = O_neighbors.view(list(O_neighbors.shape[:3]) + [3, 3])

        # Rotate into local reference frames
        dX = X_neighbors - X.unsqueeze(-2)
        dU = torch.matmul(E.unsqueeze(2), dX.unsqueeze(-1)).squeeze(-1)
        dU = F.normalize(dU, dim=-1)
        R = torch.matmul(E.unsqueeze(2).transpose(-1, -2), O_neighbors)
