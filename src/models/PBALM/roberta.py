...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        if config.is_cross_attention:
            kv_hidden_size = getattr(config, "encoder_hidden_size", config.hidden_size)
        else:
    ...
    def __init__(self, config, add_cross_attention=False):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = RobertaAttention(
                config.adapter_config, position_embedding_type="absolute"
            )
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [
                RobertaLayer(config, add_cross_attention=i in config.adapter_layer_indices)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    architecture described in *Attention is all you need*_ by Ashish Vaswani, Noam Shazeer, Niki
